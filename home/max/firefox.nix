  ...
  programs.firefox = {
    ...
    policies = {
      ExtensionSettings = {
        ...
        # Vimium: vim-style keyboard navigation. The id is the add-on's AMO
        # GUID and the URL is AMO's "latest" endpoint for its slug, so no
        # extension version is pinned here.
